...
# class TextCleaner(InputPostProcessor):
#     """文本清理处理器"""
...
#         # if current_title:
#         #     current_title = current_title.strip()
#         #     item["title"] = current_title  # 更新 item 中的 title
...
#         #     result = ''.join(item.replace('\n', '') for item in paragraph)
#         #     paragraph = [result]
#         #     item["paragraphs"] = paragraph  # 更新 item 中的 paragraphs

#         # # 递归处理子项
#         # for child in children:
#         #     self.process(child)  # 递归调用处理子项
...
#     def clean_newlines_and_merge_paragraphs(self, data):
#         if isinstance(data, list):
#             for item in data:
#                 self.clean_newlines_and_merge_paragraphs(item)
...
#             # 清理并拼接 paragraph
#             if 'paragraph' in data and isinstance(data['paragraph'], list):
#                 cleaned_paragraphs = [self.clean_text(p) for p in data['paragraph']]
#                 data['paragraph'] = [''.join(cleaned_paragraphs)]
            
#             # 递归清理 children
#             if 'children' in data and isinstance(data['children'], list):
#                 self.clean_newlines_and_merge_paragraphs(data['children'])
#         return data
...
class TextCleaner(InputPostProcessor):
    ...
    def process(self, item):
        """对输入item进行清理处理，并返回处理后的item"""
        if not item:
            return item  # 如果是None或者空，原样返回
        #print(item)
        
        return self.clean_newlines_and_merge_paragraphs(item)
    ...
    def clean_newlines_and_merge_paragraphs(self, data):
        """递归清理title、paragraph，并拼接paragraph文本"""
        if isinstance(data, list):
            for item in data:
                self.clean_newlines_and_merge_paragraphs(item)
        elif isinstance(data, dict):
            ...
            if 'paragraph' in data and isinstance(data['paragraph'], list):
                cleaned_paragraphs = [self.clean_text(p) for p in data['paragraph']]
                data['paragraph'] = ''.join(cleaned_paragraphs)  # 注意不要套一个列表！
...
class StructureNormalizer(InputPostProcessor):
    """结构数据标准化处理器"""
    ...
    def process(self, input_data):
        ...
        input_data = self.move_paragraph_to_title(input_data)
    ...
    def move_paragraph_to_title(self,data, default_title='无标题内容'):
        """
        将列表中每个字典项中paragraphs的第一个内容移动到title中（如果title为默认标题且paragraphs不为空）。
        
        参数:
        - data: list[dict]，包含'title', 'paragraphs', 'children'字段的字典组成的列表。
        - default_title: str，表示需要被替换的默认标题，默认值为'无标题内容'。
        
        返回:
        - 处理后的列表，title替换为paragraph内容，paragraphs置空。
        """
        for item in data:
            if item.get('paragraphs'):
                combined_paragraphs = "".join(item['paragraphs'])
                item['paragraphs'] = combined_paragraphs
            if item.get('title') == default_title and item.get('paragraphs'):
                item['title'] = item['paragraphs'][0]
                item['paragraphs'] = []
        return data


class FieldExtractor:
    def __init__(self, fields_to_keep):
        """
        初始化提取器，指定要保留的字段。
        
        :param fields_to_keep: List[str]，需要保留的字段名
        """
        self.fields_to_keep = fields_to_keep

    def extract(self, data_list):
        """
        从列表中的每个字典中提取指定字段，返回新的列表。
        
        :param data_list: List[Dict]，原始数据列表
        :return: List[Dict]，提取后的新列表
        """
        extracted_list = []
        for item in data_list:
            new_item = {field: item[field] for field in self.fields_to_keep if field in item}
            extracted_list.append(new_item)
    ...
    def set_fields(self, new_fields):
        ...
        :param new_fields: List[str]，新的字段名
        """
        self.fields_to_keep = new_fields
...
if __name__ == '__main__':
    ...
    test = StructureNormalizer()
    #print(test.move_paragraph_to_title(data))

    test2 = TextCleaner()
    data22 = test2.clean_newlines_and_merge_paragraphs(data)
    #print(data22)
    ...
    cleaned_data = text_cleaner.process(data)
    print(cleaned_data)
